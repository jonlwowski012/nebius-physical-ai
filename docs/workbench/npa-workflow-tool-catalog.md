...
| `workflow.groot.prepare_dataset` | `npa.workflows.groot_learning prepare-dataset` | standard LeRobot dataset (v3.0 or v2.1) at `config.source_data_uri` | per-episode GR00T dataset at `config.prepared_data_uri` plus a fail-closed dataset audit at `config.dataset_audit_uri` | no |
| `workflow.groot.prepare_split` | `npa.workflows.groot_learning prepare-split` | source GR00T LeRobot dataset, optionally restricted by an Encord curation manifest | hashed, episode-disjoint train/held-out datasets + split manifest with train-only statistics and the recorded curation selection | no |
...
| `workflow.groot.validate_checkpoints` | `npa.workflows.groot_learning validate-checkpoints` | training manifest, split manifest, baseline evaluation, every saved checkpoint | per-checkpoint validation errors, a validation curve, and a selected checkpoint (or an explicit none-beat-baseline) | yes (real `Gr00tPolicy.get_action` forwards per candidate) |
| `workflow.groot.resolve_trained_checkpoint` | `npa.workflows.groot_task_performance resolve-trained-checkpoint` | split/training manifests, uploaded trainer output, optional validation selection | step-contract-checked immutable checkpoint reference for the selected step, or the final step when no selection is supplied | no |
