...
def _dataset(root: Path) -> None:
    ...
    (root / "meta/tasks.jsonl").write_text(json.dumps({"task_index": 0, "task": "Pick up the block."}) + "\n")
...
def _two_episode_dataset(root: Path, *, with_stale_jsonl: bool = False) -> None:
    ...
    # p5 added a fail-closed audit check: lerobot_to_groot's own generated
    # modality.json declares a language annotation by default, and if the
    # source carries no task text the conversion would silently train on an
    # empty instruction. This fixture must carry real task text or every
    # materialize() test now fails that audit rather than exercising this
    # module's own logic.
...
def test_materialize_trims_synthetic_rows_to_the_clip_frame_count(tmp_path: Path) -> None:
    ...
    # variant-1 is a real encode (not the stdlib-only _fake_mp4) at the
    # fixture's declared 64x64/20fps: the geometry audit now shells to real
    # ffprobe over the whole materialized tree, and a structurally-valid but
    # undecodable MP4 reports 0x0, which the audit correctly treats as a
    # mismatch against the declared shape rather than "unprobeable".
    source = tmp_path / "source"
    _two_episode_dataset(source)
    generated = tmp_path / "generated"
    (generated / "variant-1").mkdir(parents=True)
    _real_mp4(generated / "variant-1" / "vision.mp4", width=64, height=64, fps=20, frames=1)
